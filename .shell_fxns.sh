...
{
    if [ -z ${1+x} ]; then
		echo "geanyy requires at least one argument"
    else
		geany -i "$@" &>/dev/null &
    fi
}
			...
			js)
				case ${match} in
					node)
						...
						;;
					mocha)
						mocha -u tdd --no-colors --reporter spec "${curFile}"
						;;
					browserify)
...
# found here: http://stackoverflow.com/a/4256095
# example usage: find . -type f | basename_sort
function basename_sort
{
	~/perl-bits/basename_sort.pl "${@}"
}
